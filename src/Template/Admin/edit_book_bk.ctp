						...
						<div class="col-sm-5">
							<div class="right-menu">
								<ul class="list-inline">
									...
									<li><a href="#" ng-disabled="progress() != 100" ng-click="save(false);" class="btn btn-2">Save</a></li>
									<li><a href="#" ng-disabled="progress() != 100" ng-click="save(true);" class="btn btn-4">Save & Exit</a></li>
									<li><a target="_blank" href="<?= $this->Url->build(["controller" => "book", "action" => "view", base64_encode(base64_encode($book['id'])), $book['slug']]);?>" class="btn btn-3">Preview
									</a></li>
								</ul>
							</div>
						</div>
									...
									<div class="col-sm-8">
										...
										<div class="tools-box-content-stage" ng-show="attribute_tab == 6">
											<div class="img-preview text-center">
												...
												<input class="form-control" ng-show="data['active_map'] != -1" ng-enter type="text" id="change_map_val" />
											</div>
											<div class="library_list">
												<h3>Item Library</h3>
												<ul class="tools-bottom-menu clearfix">
											...
											<div class="tools-box-content-stage-bottom template_video_attributes">
												<ul ng-if="active_sub_page == -1" class="list-inline">
													<li ng-repeat="t in page[active_page]['template_attributes']['map']  track by $index" ng-click="change_active_video($index, page[active_page]['template_attributes']['map'][$index]['value'])" ng-class="{active: data['active_video'] == $index}"  >
														<i class="fa fa-map-marker"></i>
														<span class="delete_attributes" ng-click="delete_attribute($event, $index)"><?= $this->Html->image('delete.png');?></span>
													</li>
												</ul>
												<ul ng-if="active_sub_page != -1" class="list-inline">
													<li ng-repeat="t in page[active_page]['sub_products'][active_sub_page]['template_attributes']['map']  track by $index" ng-click="change_active_video($index, page[active_page]['sub_products'][active_sub_page]['template_attributes']['map'][$index]['value'])" ng-class="{active: data['active_video'] == $index}"  >
														<i class="fa fa-map-marker"></i>
														<span class="delete_attributes" ng-click="delete_attribute($event, $index)"><?= $this->Html->image('delete.png');?></span>
													</li>
												</ul>
    ...
    function($scope, $http, $timeout) {
    	...
    	$scope.change_active_text = function(ind, txt)
        {
          ...
          $('#colorpicker').val(color);
      	  ...
      	  $( "#wslider" ).slider("value", parseFloat($scope.position.w) ? parseFloat($scope.position.w) : 0);
        };
        ...
        $scope.change_active_video = function(ind, txt)
        {
          ...
          $( "#wslider" ).slider("value", parseFloat($scope.position.w) ? parseFloat($scope.position.w) : 0);
          $( "#hslider" ).slider("value", parseFloat($scope.position.h) ? parseFloat($scope.position.h) : 0);
        };
        ...
        $scope.select_active_page = function(id, e){
          	...
          	{
          		$scope.preloader = 2;
          		html2canvas([$(".book_preview_container")[0]], {
		...
		$scope.new_attribute = function(e){
			...
			if($scope.active_sub_page == -1)
			{
				if($scope.attribute_tab == 1)
				{
					...
					$("#wslider").addClass('new_attribute');
				}
				else if($scope.attribute_tab == 2)
				{
					...
					$("#wslider").addClass('new_attribute');
					$("#hslider").addClass('new_attribute');
				}
		...
		$scope.create_new_template = function(){
			if($scope.active_sub_page == -1)
			{
				...
				$scope.page[$scope.active_page]['template_attributes'] = {'text': [], 'image': [], 'video': [], 'background': {'value': $scope.page[$scope.active_page]['page'] == 5 ? socialwall_img : '#fff'}};
			}
			else
			{
				$scope.page[$scope.active_page]['sub_products'][$scope.active_sub_page]['template'] = -1;
				$scope.page[$scope.active_page]['sub_products'][$scope.active_sub_page]['template_attributes'] = {'text': [], 'image': [], 'video': [], 'background': {'value': $scope.page[$scope.active_page]['sub_products'][$scope.active_sub_page]['page'] == 5 ? socialwall_img : '#fff'}};
			}
