            ...
            <form onsubmit="return false;" name="new_company" novalidate action="" method="post" class="register_step_form new_company" enctype="multipart/form-data">
              ...
              <div class="register_section">
                ...
                <div class="register_step_bg" ng-if="tab==3">
                  ...
                  <p>
                    <button ng-click="previous();" class="btn btn_submit left">Previous</button>
                    <button ng-disabled="company.data[6] === undefined || company.data[4] === undefined || company.data[5] === undefined" ng-click="next();" class="btn btn_submit right">Next</button>
                  </p>
                </div>
...
function calculate_percentage(total, used)
{
  ...
  buzztm.controller('AddController', ['$scope', '$http',
    function($scope, $http) {
        ...
        $scope.company = {"name": "<?= $themes['name']; ?>", "category_type": "<?= $themes['category_type']; ?>", data: <?= json_encode($data); ?>};
        $scope.tab = 1;
        $scope.clicked = false;
        $scope.templates = [];
        ...
        $scope.next = function()
        {
            ...
            if($scope.tab == 4)
            {
              ...
              $http.post('<?= $this->Url->build(["controller" => "admin", "action" => "update_theme", $id]);?>', $scope.company)
