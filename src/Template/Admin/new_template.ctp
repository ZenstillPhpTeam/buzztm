...
function calculate_percentage(total, used)
{
  ...
  buzztm.controller('AddController', ['$scope', '$http',
    function($scope, $http) {
        ...
        $scope.company = {"template_name":"ghdfghd","width":400,"height":500,"textbox":2,"image":2, "page_type": "", "category_type": ""};
        $scope.tab = 1;
        $scope.clicked = false;
        $scope.upload_button_text = 'Upload';
        var formData = 0;

        $scope.data = {template: $scope.company, image: [], text: [], background:{value: '#fff'}, active_text: -1, active_image: -1, background_st: 0};
        ...
        $scope.next = function()
        {
            ...
            if($scope.tab == 3)
            {
              ...
              $http.post('<?= $this->Url->build(["controller" => "admin", "action" => "create_template"]);?>', $scope.data)
